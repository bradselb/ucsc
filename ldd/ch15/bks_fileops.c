...
void bks_vma_open(struct vm_area_struct *vma)
{
    printk(KERN_INFO "[bks] vma_open(), virtual %08lx, physical %08lx\n", vma->vm_start, vma->vm_pgoff << PAGE_SHIFT);
}

void bks_vma_close(struct vm_area_struct *vma)
{
    printk(KERN_INFO "[bks] vma_close.\n");
}

static struct vm_operations_struct bks_vm_ops = {
    .open =  bks_vma_open,
    .close = bks_vma_close,
};
...
{
	int rc = 0;
	printk(KERN_INFO "[bks] mmap()\n");
	if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff, vma->vm_end - vma->vm_start, vma->vm_page_prot)) {
		rc = -EAGAIN;
	} else {
		vma->vm_ops = &bks_vm_ops;
		bks_vma_open(vma);
		rc = 0;
	}
	return rc;
...
// -------------------------------------------------------------------------
unsigned int bks_poll(struct file* file, struct poll_table_struct* poll_table)
{
	unsigned int rc = 0;
	return rc;
}
